...
public class Ingots {
    ...
    public static Map<String, Item> ITEMS = new HashMap<String, Item>() {{
        for (Map.Entry<String, List<String>> item : Items.ITEMS.entrySet()) {
            if (ArrayUtils.contains(item.getValue().toArray(new String[]{}), "has_ingot")) {
                put(
                    registryName(item.getKey(), null, "INGOT").toUpperCase(),
                    new Item(PROPERTIES).setRegistryName(registryName(item.getKey(), null, "INGOT"))
                );
            }
        }
    }};
