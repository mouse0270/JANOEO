...
public class Rods {
    ...
    public static Map<String, Item> ITEMS = new HashMap<String, Item>() {{
        for (Entry<String, List<String>> item : Items.ITEMS.entrySet()) {
            if (ArrayUtils.contains(item.getValue().toArray(new String[]{}), "has_rod")) {
                put(
                    registryName(item.getKey(), null, "ROD").toUpperCase(),
                    new Item(PROPERTIES).setRegistryName(registryName(item.getKey(), null, "ROD"))
                );
            }
        }
    }};
