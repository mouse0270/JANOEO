...
public class Nuggets {
    ...
    public static Map<String, Item> ITEMS = new HashMap<String, Item>() {{
        for (Entry<String, List<String>> item : Items.ITEMS.entrySet()) {
            if (ArrayUtils.contains(item.getValue().toArray(new String[]{}), "has_nugget")) {
                put(
                    registryName(item.getKey(), null, "NUGGET").toUpperCase(),
                    new Item(PROPERTIES).setRegistryName(registryName(item.getKey(), null, "NUGGET"))
                );
            }
        }
    }};
